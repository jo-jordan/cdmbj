...
<title>会员注册 - Powered By M</title>
<link href="${pageContext.request.contextPath}/css/common.css" rel="stylesheet" type="text/css" />
<link href="${pageContext.request.contextPath}/css/register.css" rel="stylesheet" type="text/css" />
<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery-1.8.3.js"></script>
<script type="text/javascript">
	var verCodeFlag = true;
	/* 表单非空校验 */
	function checkForm() {
		//用户名长度校验
		var username = document.getElementById("username").value;
		if (username.length > 20 || username.length < 5) {
			alert("用户名:" + username + "长度不在范围内(6-20).");
			return false;
		}

		//密码的强度验证
		var password = document.getElementById("password").value;
		var regex_password = /^((?=.*[0-9].*)(?=.*[A-Z].*)(?=.*[a-z].*))[_0-9A-Za-z]{6,17}$/;
		if (!regex_password.test(password)) {
			alert("密码必须包含大小写字母，数字，长度6-16位.");
			return false;
		}
		//确认密码
		...
		//验证是否存在错误信息
		//var vercode = document.getElementById("captcha");
		//if(callCheckUEP(vercode)==false){
		//	alert("验证码错误.");
		//	return false;
		//}
		...
		if (obj.value != "") {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					...
					document.getElementById("check" + obj.name + "return").innerHTML = data;
					if (obj.name == "vercode") {
			...
			xmlhttp.open("GET", "${pageContext.request.contextPath}/user_check"
					+ parameter + "?" + obj.name + "=" + obj.value, true);
			xmlhttp.send(null);
		}
	}
...
<body>
		...
		<div class="span24">
			<div class="wrap">
				<div class="main clearfix">
					...
					<form id="register_form" action="${pageContext.request.contextPath}/user_register" method="post" onsubmit="return checkForm()">
						<table>
							<tbody>
								<tr>
									<th><span class="requiredField">*</span>用户名:</th>
									<td>
										<input type="text" id="username" name="username" class="text" onblur="checkUEP(this)"> <span id="checkusernamereturn"><s:fielderror
												fieldName="username"></s:fielderror></span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td>
										<input type="password" id="repassword" class="text">
									</td>
								</tr>
								<tr>
									<th>真实姓名:</th>
								...
								<tr>
									...
									<td>
										<input type="text" id="email" name="email" class="text" onblur="checkUEP(this)"> <span id="checkemailreturn"></span>
									</td>
								</tr>
								<tr>
									<th><span class="requiredField">*</span>手机:</th>
									<td>
										<input type="text" id="phone" name="phone" class="text" onblur="checkUEP(this)"> <span id="checkphonereturn"></span>
									</td>
								</tr>
								<tr>
									<th>地址:</th>
								...
								<tr>
									<th><span class="fieldSet"> </span>性别:</th>
									<td>
										<label><input type="radio" name="sex" value="男">男</label> <label><input type="radio" name="sex" value="女">女</label> <label><input
											type="radio" name="sex" value="保密">保密</label>
									</td>
								</tr>
								<tr>
									...
									<td>
										<span class="fieldSet"> <input type="text" id="captcha" name="vercode" class="text captcha" maxlength="4" autocomplete="off"
											onkeyup="callCheckUEP(this)">
										<a href="#" onclick="changeVerCodeImg()">
												<img id="captchaImage" class="captchaImage" src="${pageContext.request.contextPath}/createVerCodeAndImg" title="点击更换验证码">点击更换验证码
											</a>
										</span><span id="checkvercodereturn"></span>
									</td>
								</tr>
								<tr>
									<th>&nbsp;</th>
								...
								<tr>
									...
									<td>
										<div id="agreement" class="agreement" style="height: 200px;">
											...
											<p>
												二、服务简介<br />本网站运用自己的操作系统通过国际互联网络为用户提供各项服务。用户必须: 1. 提供设备，如个人电脑、手机或其他上网设备。 2. 个人上网和支付与此服务有关的费用。
											</p>
											<p>
												三、用户在不得在本网站上发布下列违法信息<br />1. 反对宪法所确定的基本原则的； 2. 危害国家安全，泄露国家秘密，颠覆国家政权，破坏国家统一的； 3. 损害国家荣誉和利益的； 4. 煽动民族仇恨、民族歧视，破坏民族团结的； 5.
												破坏国家宗教政策，宣扬邪教和封建迷信的； 6. 散布谣言，扰乱社会秩序，破坏社会稳定的； 7. 散布淫秽、色情、赌博、暴力、凶杀、恐怖或者教唆犯罪的； 8. 侮辱或者诽谤他人，侵害他人合法权益的； 9. 含有法律、行政法规禁止的其他内容的。
											</p>
											<p>
												四、有关个人资料<br />用户同意: 1. 提供及时、详尽及准确的个人资料。 2. 同意接收来自本网站的信息。 3. 不断更新注册资料，符合及时、详尽准确的要求。所有原始键入的资料将引用为注册资料。 4.
												本网站不公开用户的姓名、地址、电子邮箱和笔名。除以下情况外: a) 用户授权本站透露这些信息。 b) 相应的法律及程序要求本站提供用户的个人资料。
											</p>
											<p>
												五、服务条款的修改<br />本网站有权在必要时修改服务条款，一旦条款及服务内容产生变动，本网站将会在重要页面上提示修改内容。如果不同意所改动的内容，用户可以主动取消获得的本网站信息服务。如果用户继续享用本网站信息服务，则视为接受服务条款的变动。
											</p>
											<p>
												六、用户隐私制度<br />尊重用户个人隐私是本网站的一项基本政策。所以，本网站一定不会在未经合法用户授权时公开、编辑或透露其注册资料及保存在本网站中的非公开内容，除非有法律许可要求或本网站在诚信的基础上认为透露这些信息在以下四种情况是必要的: 1.
												遵守有关法律规定，遵从本网站合法服务程序。 2. 保持维护本网站的商标所有权。 3. 在紧急情况下竭力维护用户个人和社会大众的隐私安全。 4. 符合其他相关的要求。
											</p>
											<p>
											...
											<p>
												十一、用户管理<br />用户单独承担发布内容的责任。用户对服务的使用是根据所有适用于本站的国家法律、地方法律和国际法律标准的。用户必须遵循: 1. 使用网络服务不作非法用途。 2. 不干扰或混乱网络服务。 3.
						...
						<div class="login">
							...
							<dl>
								...
								<dd>
									立即登录即可体验在线购物！
									<a href="${pageContext.request.contextPath}/user_goLoginPage">立即登录</a>
								</dd>
							</dl>
						</div>
					</form>
	...
	<div id="_my97DP" style="position: absolute; top: -1970px; left: -1970px;">
		<iframe style="width: 190px; height: 191px;" src="./会员注册 - Powered By Mango Team_files/My97DatePicker.htm"
			style="frameborder:0;border:0;scrolling:no"></iframe>
	</div>
</body>
</html>