...
public class VerCodeImg extends ActionSupport {
	public String createVerCodeImg() throws Exception {
		int width = 95;
		int height = 30;
		// 生成画布进内存
		...
		// 设置画笔颜色
		pen.setColor(getRandColor(1,255));
		// 画一个矩形
		...
		for (int i = 0; i < 4; i++) {
			pen.setColor(getRandColor(1,255));
			pen.setFont(new Font("Consolo", Font.BOLD, 20));
			// 旋转字母 rotate(double theta, double x2, double y2); theta为弧度 theta =
			// angle * Math.PI / 180
			int angle = random.nextInt(60) - 30;
			double theta = angle * Math.PI / 180;
			pen.rotate(theta, x, y);
			// 随机生成验证码
			int index = random.nextInt(words.length());
			char ch = words.charAt(index);
			sb.append(ch);
			pen.drawString("" + ch, x, y);
			pen.rotate(-theta, x, y);
			x += 20;
			// 画干扰线
			pen.setColor(getRandColor(100,255));
		...
		System.out.println("验证码生成session中的验证码: "+request.getSession().getAttribute("verCode"));
